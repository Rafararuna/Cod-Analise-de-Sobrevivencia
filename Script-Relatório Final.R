...
### Abrindo e Organizando o banco de dados:
banco_autism <- read.csv2("C:/Users/jgararuna/Downloads/Trabalho de Sobrevivência/autism.csv")


names(banco_autism)[1:11] <- c("Nome do Estado", "Ano de Adoção", "Tempo de Falha", "Falha/Censura", 
                               "Gov_Dem", "Gov_Rep", "Dif_Dem-Rep (%)", "Ideologia do Cidadão",
...
nome <- c("Nome do Estado", "Ano de Adoção", "Tempo de Falha", "Falha/Censura", "Gov_Dem", "Gov_Rep", 
          "Dif_Dem-Rep (%)", "Ideologia do Cidadão", "Ideologia do Governo",
          "Profissionalismo Legislativo", "População")

descricao <- c("Nome do Estado americano","Ano da adoção da política de autismo",
               "Tempo até a adoção da política de autismo (Tempo inicial é quando o primeiro estado adota a política)",
               "Se o estado adotou a política (1 - adotou/ 0 - não adotou)",
               "Se o governo é democrata (1 - democrata/ 0 - não democrata)",
               "Se o governo é republicano (1 - republicano / 0 - não republicano)",
               "Diferença percentual entre os votos republicanos e democratas nas eleições para Governador do Estado",
               "Medida da ideologia dos cidadãos (0 - Mais Conservador/ 100 - Mais liberal)",
               "Medida da ideologia do Governo do Estado (0 - Mais Conservador/ 100 - Mais liberal)",
               "Medida do profissionalismo legislativo (variável baseada na assembleia de cada estado -
               número de projetos aprovados, horas trabalhadas, salário dos funcionários, etc.)",
               "População do Estado no ano de aprovação da lei ou no último ano")
...
nX1 <- c("Média","Mínimo","1º Quartil","Mediana","3º Quartil","Máximo","Variância","Desvio Padrão",
...
mX1 <- c(round(c(meanX1,minX1,q1X1,medianX1,q3X1,maxX1,varX1,sdX1,skewX1,kurtX1,coef.varX1),2))
nX1 <- c("Média","Mínimo","1º Quartil","Mediana","3º Quartil","Máximo","Variância",
         "Desvio Padrão","Assimetria","Curtose","Coeficiente de Variação(%)")
variX1 <- data.frame(Medidas = nX1, Valores = mX1)
kable(variX1,caption = "Medidas de posição, variabilidade, assimetria e curtose")
...
ggplot(banco_autism, aes(x=banco_autism$`Profissionalismo Legislativo`)) + 
  geom_histogram(colour="white", fill="#7acedc", binwidth = 0.1)+
...
ggplot(banco_autism, aes(x=log(banco_autism$População))) +
  geom_histogram(colour="white", fill="#7acedc", bins = 10)+
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)",mark.time = T, col = "#7acedc", 
     main = "Gráfico da Função de Sobrevivência Estimada", cex.main = 1, lwd = 2.5)
...
plot(stepfun(KM$time,c(0,HHt)),do.points = F,xlab="Tempo", 
     ylab="H(t)",main = "Estimação por KM", col = "#7acedc", lwd=2.5, cex.main = 1)

#função de risco acumulada estimada por NA
plot(KM,conf.int=F, fun="cumhaz", xlab="Tempo", ylab="H(t)", col="#7acedc", 
     main = "Estimação por NA",lwd = 2.5 , cex.main = 1)
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#bd1313","#1386bf"),
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência por Ideologia do Governo",
     cex.main = 1)
...
#Teste Semelhante à Wilcoxon (rho=1)
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#bd1313","#1386bf"),
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência por Ideologia do Cidadão", 
     cex.main = 1)
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#bd1313","#1386bf"), 
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência por partido do governo",
     cex.main = 1)
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#73b2ff","#212962"),
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência pela Diferença Percentual dos Votos")
...
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#73b2ff","#212962"),
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência pelo Profissionalismo Legislativo")
...
#summary(KM)
plot(KM,conf.int=F, xlab="Tempo", ylab="S(t)", lty = c(1,2), col = c("#73b2ff","#212962"),
     mark.time = T, lwd = 2.5, main = "Função de Sobrevivência pela População")
...
#Teste Semelhante à Wilcoxon (rho=0)
...
mwe <- survreg(Surv((tempo+0.01),censura)~1, data=banco_autism, dist = "weibull")
...
#Estimativas da Weibull Discreta
alphawd <- v1$par[1]
gammawd <- v1$par[2]
...
#Estimativas da Log-Logística Discreta
alphalld <- v1$par[1]
gammalld <- v1$par[2]
...
#Criando as funções de sobrevivência de cada modelo candidato
KM<-survfit(Surv(tempo,censura)~1,conf.int=F)
time <- KM$time

## Verificando ajuste com Função de sobrevivência
sw <- exp(-(time/alphaw)^gammaw) #weibull

## Verificando ajuste com Função de sobrevivência
q <- exp(-1/(alphawd^gammawd))
swd <- q^((time+1)^gammawd) #weibull discreta

## Verificando ajuste com Função de sobrevivência
sll <- 1/(1+(time/alphall)^gammall) #log-logpistica

## Verificando ajuste com Função de sobrevivência
slld <- (1/(1+((time+1)/alphalld)^gammalld)) #log-logística discreta
...
#AIC, AICc e BIC - Weibull Discreto
...
#AIC, AICc e BIC - Log-Logística Discreto
...
d <- data.frame("Modelo" = c("Weibull", "Weibull Discreta", "Log-Logística", "Log-Logística Discreta"), 
                AIC = c(AICw, AICwd, AICll, AIClld), "AICc" = c(AICcw,AICcwd,AICcll, AICclld),
                "BIC" = c(BICw,BICwd, BICll,BIClld))
...
fit2 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~  
                banco_modelo$Gov_Dem)

fit3 <- coxph(Surv(sem_na$`Tempo de Falha`, sem_na$`Falha/Censura`) ~  sem_na$`Ideologia do Cidadão`)

fit4 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                banco_modelo$`Dif_Dem-Rep (%)`)

fit5 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                banco_modelo$`Ideologia do Governo`)

xx <- banco_modelo[complete.cases(banco_modelo), ]
fit5b <- coxph(Surv(xx$`Tempo de Falha`, xx$`Falha/Censura`) ~  xx$`Ideologia do Governo`)

fit6 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                banco_modelo$`Profissionalismo Legislativo`)

fit7 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                banco_modelo$População)



df <- data.frame("Variáveis" = c("Tipo de Governo","Ideologia do Cidadão",
                                 "Diferença dos Votos (%)","Ideologia do Governo",
                                 "Profissionalismo Legislativo", "População"),
...
p <- data.frame("ll" = c(fit1$loglik,fit2$loglik[2], fit3$loglik[2],
                         fit4$loglik[2], fit5$loglik[2], 
                         fit6$loglik[2], fit7$loglik[2]),
                "lll"= c(0,2*(fit2$loglik[2] - fit2$loglik[1]),
                         2*(fit3$loglik[2] - fit3$loglik[1]), 
                         2*(fit4$loglik[2] - fit4$loglik[1]),
                         2*(fit5$loglik[2] - fit5$loglik[1]),
                         2*(fit6$loglik[2] - fit6$loglik[1]),
                         2*(fit7$loglik[2] - fit7$loglik[1])),
                "llll" = c(0,1-pchisq(2*(fit2$loglik[2] - fit2$loglik[1]),1),
                           1-pchisq(2*(fit3$loglik[2] - fit3$loglik[1]),1),
                           1-pchisq(2*(fit4$loglik[2] - fit4$loglik[1]),1),
                           1-pchisq(2*(fit5$loglik[2] - fit5$loglik[1]),1),
                           1-pchisq(2*(fit6$loglik[2] - fit6$loglik[1]),1),
                           1-pchisq(2*(fit7$loglik[2] - fit7$loglik[1]),1)))
...
# Modelo de Covariáveis Categorizadas
...
names(df) <- c("Variável Categorizada","Estimativa","Erro Padrão",
               "Estatística","P-valor")
row.names(df) <- c( "Modelo 2 discretizado","Modelo 3 discretizado", "Modelo 4 discretizado",
                    "Modelo 5 discretizado", "Modelo 6 discretizado")
kable(df, caption = "Modelos com uma variável")



g <- data.frame("ll" = c(fit1$loglik,fit2d$loglik[2], 
                         fit3d$loglik[2], fit4d$loglik[2],
                         fit5d$loglik[2], fit6d$loglik[2]),
                "lll"= c(0,2*(fit2d$loglik[2] - fit2d$loglik[1]),
                         2*(fit3d$loglik[2] - fit3d$loglik[1]),
                         2*(fit4d$loglik[2] - fit4d$loglik[1]),
                         2*(fit5d$loglik[2] - fit5d$loglik[1]),
                         2*(fit6d$loglik[2] - fit6d$loglik[1])),
                "llll" = c(0,1-pchisq(2*(fit2d$loglik[2] - fit2d$loglik[1]),1)
                           ,1-pchisq(2*(fit3d$loglik[2] - fit3d$loglik[1]),1),
                           1-pchisq(2*(fit4d$loglik[2] - fit4d$loglik[1]),1),
                           1-pchisq(2*(fit5d$loglik[2] - fit5d$loglik[1]),1),
                           1-pchisq(2*(fit6d$loglik[2] - fit6d$loglik[1]),1)))
...
# Modelo com todas variáveis significativas

fit8 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                banco_modelo$Gov_Dem + banco_modelo$`Ideologia do Governo` + banco_modelo$População)


df <- data.frame("Variável" = c("Tipo de Governo",
                                "Ideologia do Governo","População"),
                 "Estimativas"= c(-0.2000, 0.0319,"1.045e-08"),
...
## Teste de razão de verossimilhança entre o modelo final provisório e modelos removendo as variáveis uma a uma
# Modelos

fitg1 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                 banco_modelo$População+banco_modelo$`Ideologia do Governo`)

fitg2 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                 banco_modelo$`Ideologia do Governo`+banco_modelo$Gov_Dem)

fitg3 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                 banco_modelo$População+banco_modelo$Gov_Dem)


# Modelo sem Tipo de Governo
df <- data.frame("Variável" = c("População",
                                "Ideologia do Governo"),
                 "Estimativas"= c("1.002e-08", 0.0279),
...
# Modelo sem População
df <- data.frame("Variável" = c("Ideologia do Governo",
                                "Tipo de Governo"),
                 "Estimativas"= c(0.0331, -0.1838),
...
df <- data.frame("Variável" = c("População",
                                "Tipo de Governo"),
                 "Estimativas"= c("1.920e-08", 0.5534),
...
# TRV dos modelos com duas variáveis contra o modelo final provisório
g <- data.frame("ll" = c(fit8$loglik[2],fitg1$loglik[2],fitg2$loglik[2], fitg3$loglik[2]),
                "lll"= c(0,2*(fit8$loglik[2] - fitg1$loglik[2]),
                         2*(fit8$loglik[2] - fitg2$loglik[2]),
                         2*(fit8$loglik[2] - fitg3$loglik[2])),
                "llll" = c(0,1-pchisq(2*(fit8$loglik[2] - fitg1$loglik[2]),1),
                           1-pchisq(2*(fit8$loglik[2] - fitg2$loglik[2]),1),
                           1-pchisq(2*(fit8$loglik[2] - fitg3$loglik[2]),1)))
...
## Modelo com 2 variáveis x modelos com 1 variável

# Modelos com uma variável contra o modelo sem Tipo de Governo
g <- data.frame("ll" = c(fitg1$loglik[2],fit7$loglik[2],fit5$loglik[2]),
                "lll"= c(0,2*(fitg1$loglik[2] - fit7$loglik[2]),
                         2*(fitg1$loglik[2] - fit5$loglik[2])),
                "llll" = c(0,1-pchisq(2*(fitg1$loglik[2] - fit7$loglik[2]),1),
                           1-pchisq(2*(fitg1$loglik[2] - fit5$loglik[2]),1)))
...
# Modelos com uma variável contra o modelo sem População
g <- data.frame("ll" = c(fitg2$loglik[2],fit2$loglik[2],fit5$loglik[2]),
                "lll"= c(0,2*(fitg2$loglik[2] - fit2$loglik[2]),
                         2*(fitg2$loglik[2] - fit5$loglik[2])),
                "llll" = c(0,1-pchisq(2*(fitg2$loglik[2] - fit2$loglik[2]),1),
                           1-pchisq(2*(fitg2$loglik[2] - fit5$loglik[2]),1)))
...
## Modelo Final Provisório x Modelo Completo

# Modelo 1

mod_tot1 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                    banco_modelo$Gov_Dem + banco_modelo$`Ideologia do Governo` +
                    banco_modelo$`Dif_Dem-Rep (%)` + banco_modelo$`Ideologia do Cidadão` +
                    banco_modelo$`Profissionalismo Legislativo`)

# Modelo 2
mod_tot2 <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~  
                    banco_modelo$Gov_Dem + banco_modelo$`Ideologia do Governo` + 
                    banco_modelo$`Dif_Dem-Rep (%)` + 
                    banco_modelo$`Ideologia do Cidadão` + banco_modelo$População)

# Modelo completo 1
df <- data.frame("Variável" = c("Tipo de Governo",
                                "Ideologia do Governo","Diferença de Votos (%)",
                                "Ideologia do Cidadão","Profissionalismo Legislativo"),
                 "Estimativas"= c(0.4008, -0.0127,-0.0033,0.0094,-1.4231),
...
df <- data.frame("Variável" = c("Tipo de Governo",
                                "Ideologia do Governo","Diferença de Votos (%)",
                                "Ideologia do Cidadão","População"),
                 "Estimativas"= c(0.3338, -0.0099,-0.0037,0.0067,"-7.125e-09"),
...
# Modelo 4 com contra os modelos Completos
g <- data.frame("ll" = c(fit5b$loglik[2],mod_tot1$loglik[2],mod_tot2$loglik[2]),
                "lll"= c(0,2*(mod_tot1$loglik[2] - fit5b$loglik[2]),
                         2*(mod_tot2$loglik[2] - fit5b$loglik[2])),
                "llll" = c(0,1-pchisq(2*(mod_tot1$loglik[2] - fit5b$loglik[2]),1),
                           1-pchisq(2*(mod_tot2$loglik[2] - fit5b$loglik[2]),1)))
...
mod_tot1a <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~  
                     banco_modelo$Gov_Dem + banco_modelo$`Ideologia do Governo` + 
                     banco_modelo$`Ideologia do Cidadão` + banco_modelo$`Profissionalismo Legislativo`)

mod_tot1b <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$Gov_Dem +   banco_modelo$`Ideologia do Cidadão` + 
                     banco_modelo$`Profissionalismo Legislativo`)

mod_tot1c <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$Gov_Dem  + banco_modelo$`Profissionalismo Legislativo`)

mod_tot1d <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$Gov_Dem)
...
mod_tot2a <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~  
                     banco_modelo$Gov_Dem + banco_modelo$`Ideologia do Governo` + 
                     banco_modelo$`Ideologia do Cidadão` + banco_modelo$População)

mod_tot2b <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$Gov_Dem +   banco_modelo$`Ideologia do Cidadão`+
                     banco_modelo$`Ideologia do Governo` )

mod_tot2c <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$Gov_Dem +    banco_modelo$`Ideologia do Governo` )

mod_tot2d <- coxph(Surv(banco_modelo$`Tempo de Falha`, banco_modelo$`Falha/Censura`) ~ 
                     banco_modelo$`Ideologia do Governo` )
...
vet_est <- c(vet_beta1[1]/vet_epp_beta1[1], vet_beta1[2]/vet_epp_beta1[2],
             vet_beta1[3]/vet_epp_beta1[3], vet_beta1[4]/vet_epp_beta1[4], 
             vet_beta1[5]/vet_epp_beta1[5]) 
...
x <- round(x,3)
x <- cbind(c("Tipo de Governo", "Dif. Percentual (%)", "Id. do Cidadão",
             "Id. do Governo", "Prof. Legislativo"), x)
colnames(x) <- c("Variável", "Estimativa", "Gamma", "EP Beta1", "EP Gamma",
                 "Estatística", "P-Valor")
...
p <- data.frame("ll" = c(vet_logv[1],vet_logv[2], vet_logv[3], 
                         vet_logv[4], vet_logv[5], vet_logv[6]),
                
                "lll"= c(0,2*(vet_logv[2] - vet_logv[1]),
                         2*(vet_logv[3] - vet_logv[1]),
                         2*(vet_logv[4] - semna_logv),
                         2*(vet_logv[5] - vet_logv[1]),
                         2*(vet_logv[6] - vet_logv[1])),
                
                "llll" = c(0,1-pchisq(2*(vet_logv[2] - vet_logv[1]),1),
                           1-pchisq(2*(vet_logv[3] - vet_logv[1]),1),
                           1-pchisq(2*(vet_logv[4] - semna_logv),1),
                           1-pchisq(2*(vet_logv[5] - vet_logv[1]),1),
                           1-pchisq(2*(vet_logv[6] - vet_logv[1]),1)))
...
colnames(p) <- c("LogL-Verossimilhança","Estatística", "P-valor")
rownames(p) <- c("Modelo Simples", "Tipo de Governo", "Diferença de Votos", 
                 "Ideologia do Cidadão",
...
vetD_est <- c(vetD_beta1[1]/vetD_epp_beta1[1], vetD_beta1[2]/vetD_epp_beta1[2],
              vetD_beta1[3]/vetD_epp_beta1[3], vetD_beta1[4]/vetD_epp_beta1[4],
              vetD_beta1[5]/vetD_epp_beta1[5]) 
...
rownames(x) <- c("Modelo 2 Discretizado", "Modelo 3 Discretizado", "Modelo 4 Discretizado", 
                 "Modelo 5 Discretizado", "Modelo 6 Discretizado")
kable(x,align = 'c', caption = "Modelos com uma variável")
...
p <- data.frame("ll" = c(vet_logv[1],vetD_logv[1], vetD_logv[2], vetD_logv[3],
                         vetD_logv[4], vetD_logv[5]),
                
                "lll"= c(0,2*(vetD_logv[1] - vet_logv[1]),2*(vetD_logv[2] - semna_logv),
                         2*(vetD_logv[3] - vet_logv[1]),2*(vetD_logv[4] - vet_logv[1]),
                         2*(vetD_logv[5] - vet_logv[1])),
                
                "llll" = c(0,1-pchisq(2*(vetD_logv[1] - vet_logv[1]),1),
                           1-pchisq(2*(vetD_logv[2] - semna_logv),1),
                           1-pchisq(2*(vetD_logv[3] - vet_logv[1]),1),
                           1-pchisq(2*(vetD_logv[4] - vet_logv[1]),1),
                           1-pchisq(2*(vetD_logv[5] - vet_logv[1]),1)))
...
## Modelo Completo x Modelo Final Provisório
...
vet_pvalor_comp <- c(pnorm(vet_estw_comp[1], lower.tail = T)*2,
                     pnorm(vet_estw_comp[2], lower.tail = F)*2,
                     pnorm(vet_estw_comp[3], lower.tail = F)*2, 
                     pnorm(vet_estw_comp[4], lower.tail = T)*2)
...
df <- round(df,4)
df <- cbind(c("Tipo de Governo","Diferença de Votos (%)", 
              "Ideologia do Governo","Profissionalismo Legislativo"),df)
...
vetB_estw <- c(0,vetB_betas[2]/vetB_epp_betas[2],vetB_betas[3]/vetB_epp_betas[3], 
               vetB_betas[4]/vetB_epp_betas[4], vetB_betas[5]/vetB_epp_betas[5]) 
vetB_pvalorw <- c(0,2*(1-pnorm(abs(vetB_estw[2]))), 2*(1-pnorm(abs(vetB_estw[3]))),
                  2*(1-pnorm(abs(vetB_estw[4]))),
...
vero_back_pl <- function(para){
  ...
  alfa <- exp(beta0+beta1*banco_modelo$`Dif_Dem-Rep (%)`
              +beta2*banco_modelo$`Ideologia do Governo`
              +beta3*banco_modelo$`Profissionalismo Legislativo`)
...
vetB_estw <- c(0,vetB_betas[2]/vetB_epp_betas[2],vetB_betas[3]/vetB_epp_betas[3], 
               vetB_betas[4]/vetB_epp_betas[4]) 
vetB_pvalorw <- c(0,2*(1-pnorm(abs(vetB_estw[2]))), 2*(1-pnorm(abs(vetB_estw[3]))),
                  2*(1-pnorm(abs(vetB_estw[4]))))
vetB_gamma <- round(vetB_gamma,3)


x <- data.frame(round(vetB_betas,3),round(vetB_epp_betas,3),round(vetB_estw,3),
                round(vetB_pvalorw,3))
x <- rbind(x,vetB_gamma)
colnames(x) <- c("Coeficientes Estimados", "Erro Padrão", "Estatística","P-Valor")
...
resid_deviance_par <- sign(resid_martingal_par)*sqrt(-2*(resid_martingal_par + 
                                                           banco_modelo$`Falha/Censura`*
                                                           log(banco_modelo$`Falha/Censura` - 
                                                                 resid_martingal_par)))
...
resid_deviance_back <- sign(resid_martingal_back)*sqrt(-2*(resid_martingal_back +
                                                             banco_modelo$`Falha/Censura`*
                                                             log(banco_modelo$`Falha/Censura` - 
                                                                   resid_martingal_back)))
